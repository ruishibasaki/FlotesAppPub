...
LNSMetaheuristic::solve(){
    ...
    while(++iters <= 200000 && breakCriteria<10000){
...
LNSMetaheuristic::fleet_min(){
    ...
    while(++iters <= 200000 && noImprov<10000){
...
void
LNSMetaheuristic::storeSolution(Solution& sol){
    if(!sol.feas()) return;
    double maxCost = 0;
    size_t idmax =vecSol.size()+1;
    size_t idfree =vecSol.size()+1;
    for (size_t i = vecSol.size(); i--;){
        Solution& stored = vecSol[i];
        if(!stored.feas()){
            idfree = i;
        }else if(fabs(stored.totalDistance - sol.totalDistance) > 1e-4 ||
            fabs(stored.totalDuration - sol.totalDuration)> 1e-4){
            if(maxCost<stored.cost()){
                maxCost = stored.cost();
                idmax = i;
            }
        }else{
    ...
    if(idfree<vecSol.size())vecSol[idfree] = sol;
    else if(idmax<vecSol.size()){
        if(sol.cost() < vecSol[idmax].cost()){
            vecSol[idmax] = sol;
        }
    }
}
